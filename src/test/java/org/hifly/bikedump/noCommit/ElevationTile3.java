...
public class ElevationTile3 {
    ...
    public ElevationTile3(RandomAccessFile file) {
        this.file = file;
    }
    ...
    public static void main(String[]args) throws Exception {
        File file = new File("/home/hifly/Dropbox/Docs/CYCLING_MTB/ROUTE/elevation_stats/agnoli2014_ele.gpx");
        BufferedWriter output = new BufferedWriter(new FileWriter(file));
        System.out.println("Parsing "+file.getName());
        com.topografix.gpx.x1.x0.GpxDocument doc = com.topografix.gpx.x1.x0.GpxDocument.Factory.parse(new File("/home/hifly/Dropbox/Docs/CYCLING_MTB/ROUTE/bestfromweb/agnoli2014.gpx"));
        //obtain a GpxDocument
        GpxDocument.Gpx gpx = doc.getGpx();
        GpxDocument.Gpx.Trk[] tracks = gpx.getTrkArray();
        for(GpxDocument.Gpx.Trk track:tracks) {
            Track resultTrack = new Track();
            GpxDocument.Gpx.Trk.Trkseg lastSegment = track.getTrksegArray(track.getTrksegArray().length - 1);
            GpxDocument.Gpx.Trk.Trkseg[] segments = track.getTrksegArray();
            for(GpxDocument.Gpx.Trk.Trkseg segment:segments) {
                ...
                for (int i = 0; i < trackPointsLenght; i++) {
                    final GpxDocument.Gpx.Trk.Trkseg.Trkpt w = segment.getTrkptArray(i);
                    RandomAccessFile raf = null;
                    if(w.getLat()!=null && w.getLon()!=null) {
                        raf = new RandomAccessFile("/home/hifly/projects/geomapviewer/src/main/resources/hgt/N41E013.hgt", "rw");
                        ElevationTile3 tile = new ElevationTile3(raf);
